...
static struct option const long_options[] =
    {
        ...
        // ADDED
        {"display", no_argument, NULL, READABLE_OPTION},
...
decode_switches(int argc, char **argv)
{
    ...
    if ((time_type == time_ctime || time_type == time_atime || time_type == time_btime) && !sort_type_specified && format != long_format)
    {
        sort_type = sort_time;
    }
            ...
            bool command_line_arg, char const *dirname)
{
    ...
    if (command_line_arg || print_hyperlink || format_needs_stat
        ...
        || ((print_inode || format_needs_type) && (type == symbolic_link || type == unknown) && (dereference == DEREF_ALWAYS || color_symlink_as_referent || check_symlink_mode))
...

    {
        ...
        if (S_ISLNK(f->stat.st_mode) && (format == long_format || check_symlink_mode))
        {
            ...
            if (linkname && (file_type <= indicator_style || check_symlink_mode) && stat_for_mode(linkname, &linkstats) == 0)
            {
                f->linkok = true;
...
{
    return f->filetype == directory || f->filetype == arg_directory || S_ISDIR(f->linkmode);
}
...
print_current_files(void)
{
    ...
    case long_format:
        ...
        if (extra_readable)
        {
            printf("Type | User | Group | World | Link Count | Owner | Associated Group | Size(bytes) | Last Modified | Name\n");
        }
...
{
    // ADDED
    // more accurately - change size of modebuf from 12 to 15
    char modebuf[15];
    char buf
    ...
    if (f->stat_ok)
        // ADDED extra_readable flag
        // Check filemode.c for more additions
        filemodestring(&f->stat, modebuf, extra_readable);
    else
    {
    ...
    *p = '\1';
    ...
    if (f->stat_ok && btime_ok && localtime_rz(localtz, &when_timespec.tv_sec, &when_local))
    {
